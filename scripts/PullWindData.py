...
import pandas as pd
import os
from DataCleaning import generate_time_labels, generate_time_lists
...
def SavetoDataFramesDir(DataFrame: pd.DataFrame, filename: str) -> None:
    script_dir = os.path.abspath(os.path.dirname(__file__))
    root_folder = os.path.abspath(os.path.join(script_dir, '..'))
    target_folder = os.path.abspath(os.path.join(root_folder, 'DataFrames'))
...
def BuildWindDF(DATE: str, START_TIME: str, END_TIME: str) -> pd.DataFrame:
    ...
    SavetoDataFramesDir(WindDF, 'WindDF.csv')
    return WindDF
